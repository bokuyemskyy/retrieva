...
def get_rag() -> RAG:
    if "rag" not in st.session_state:
        ...
        st.session_state.rag.set_llm(
            LLMConfig(provider="ollama", model_name="qwen2.5:3b")
        )
        st.session_state.rag.set_vlm(
            VLMConfig(provider="ollama", model_name="moondream:v2")
        )
...
def main():
    ...
    if col3.button(
        "Delete all workspaces", width="stretch", disabled=len(workspaces) == 0
    ):
